...
export COMMIT_BODY="Sync profiles with e2e-demo-profile repo"
git config --global user.email "automation@example.com"
git config --global user.name "AutomationBot" 
cd $(python ./scripts/automation/automation_config.py "update-cd-folder")
...
if [ -z "$(git status --porcelain)" ]; then 
  ...
  if [ -z "$(git status --untracked-files=no --porcelain)" ]; then 
     ...
     remote=$(python ./scripts/automation/automation_config.py "update-cd-remote")
